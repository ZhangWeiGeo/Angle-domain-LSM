...
##step1:
###vertical profile
sort_num            = 5;  sort_x_beg=x_beg+10; sort_x_interval=100;
...
for iter_ in range(0, len(iter_num_arr)):
    ...
    angle_2d_list = [-40, -30, -20, -10, 0, 90]
    
    for idx in range(0, len(angle_2d_list) ):
        
        angle = angle_2d_list[idx]
        ...
        imshow_and_write_km(output, eps_name, data_name, full_model=False, velocity=False, plot_min=0.5*output.min(), plot_max=0.5*output.max());
        ...
        file = P.return_bin_name(output.T)
